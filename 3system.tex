...
Our vision for the long term is that of an open educational ecosystem in which different creators can share information among their applications in order to maximize the level of personalization that learners percieve. 

A central API \cite{Lungu16} would function as a multiplexer between reader applicatins -- which allow the readers to interact with texts in their preferred context (eBook reader, web browser, news reader, etc.), and vocabulary trainers -- which allow the readers to practice intensely vocabulary exercises. 
...
To show the potential of such an ecosystem we first implemented ourselves the most basic components of such an ecosystem which together provide the full experience of a {\em personalized language textbook}: 

\begin{description}

  \item [Article Reader] -- consists of 1) an article browser that presents articles from sources that the user registers their interest in and 2) a web-based interactive text reader which provides seamless translations
  
  \item [Vocabulary Trainer] -- which consists of an exercise platform which generates interactive vocabulary practice exercises based on the past reading experience of the learner
